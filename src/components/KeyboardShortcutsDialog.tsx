  ...
  return createPortal(
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative bg-vinyl-surface border border-vinyl-border rounded-2xl shadow-2xl w-full max-w-md animate-fade-in">
        ...
        <div className="p-4">
          <div className="space-y-1">
            {shortcuts.map((shortcut) => (
              <div
                key={shortcut.key}
                className="flex items-center justify-between py-2.5 px-3 rounded-lg hover:bg-vinyl-border/30 transition-colors"
              >
                <span className="text-vinyl-text">{shortcut.description}</span>
                <kbd className="px-3 py-1.5 bg-vinyl-border/50 border border-vinyl-border rounded-lg text-sm font-mono text-vinyl-text-muted min-w-[3rem] text-center">
                  {shortcut.key}
                </kbd>
              </div>
            ))}
        ...
        {/* Footer */}
        <div className="px-6 py-4 border-t border-vinyl-border bg-vinyl-border/20 rounded-b-2xl">
